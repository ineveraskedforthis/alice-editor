...
std::string parse_tag(std::string_view tag, int32_t line, error_handler& err) {
	if(tag.length() != 3) {
...
std::string parse_date(std::string_view content, int32_t line, error_handler& err) {
	...
	return std::string(year_start, day_end - year_start);
}
